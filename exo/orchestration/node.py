  ...
  async def process_inference_result(
    ...
    if shard.is_last_layer():
      ...
      if generation_options and generation_options.stop:
        ...
        current_text = self.decodable_buffer[request_id] + await self.inference_engine.decode(shard, [token.item()])
        ...
        for stop_seq in stop_sequences:
          if text_to_check.endswith(stop_seq):
            if generation_options.include_stop:
              ...
              split_pos = len(current_text)
            else:
              emitted_text = current_text[:-len(stop_seq)]

            # The stop sequence does not in general line up with token boundaries so to keep the `broadcast_result` type
            # consistent we re-encode the text up to the split point possibly resulting in different tokens being
            # emitted than what was originally generated by the model. These will be decoded again by the ChatGPT api
            # before being returned to the client.
            pre_stop_tokens = await self.inference_engine.encode(shard, emitted_text, add_special_tokens=False)
            self.decodable_buffer[request_id] = emitted_text
            stop_sequence_found = True
            break

      # If we have not found a stop sequence, append the token to the buffer
      ...
      if is_eos_token:
        ...
        finish_reason = "stop"
      elif generated_tokens >= self.max_generate_tokens:
        finish_reason = "length"
      elif generation_options and generation_options.max_completion_tokens and generated_tokens >= generation_options.max_completion_tokens:
        finish_reason = "length"
